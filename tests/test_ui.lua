...
local function testInitializeReturnsFrames()
    ...
    assertEquals(DeathpoolLog.frameStrata, Deathpool.frameStrata, "history log should share the main frame strata")
    assertEquals(DeathpoolLog:GetFrameLevel(), Deathpool:GetFrameLevel(), "history log should share the main frame level")
    assertEquals(DeathpoolLog.toplevel, false, "history log should not use special top-level z ordering")
